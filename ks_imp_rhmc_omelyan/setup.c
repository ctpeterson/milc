...
readin(int prompt)
{
  ...
  if(mynode()==0) {
    ...
    IF_OK status += 
      get_f(stdin,prompt,"trajectory_length",&param.trajectory_length);
    ...
    IF_OK status += 
      get_i(
        stdin,
        prompt,
        "outer_fermion_steps_per_trajectory",
        &param.n_outer_fermion_steps
      );
    IF_OK status += 
      get_i(
        stdin,
        prompt,
        "inner_gauge_steps_per_fermion_gauge_update",
        &param.n_inner_gauge_steps
      );
  ...
  n_outer_fermion_steps = param.n_outer_fermion_steps;
  n_inner_gauge_steps = param.n_inner_gauge_steps;
  trajectory_length = param.trajectory_length;
  for(i = 0; i< n_pseudo; i++){
